...
sudo docker run --name mysql --restart always -p 3306:3306 -e MYSQL_ROOT_PASSWORD=yansong -e MYSQL_ROOT_HOST=% -e TZ="Asia/Shanghai" -v /home/ubuntu/mysql/data:/var/lib/mysql -v /home/ubuntu/mysql/mysql-files:/var/lib/mysql-files -v /home/ubuntu/mysql/conf.d:/etc/mysql/conf.d -d mysql:8.0.27
sudo docker run --name phpmyadmin -d -e PMA_ARBITRARY=1 -p 5200:80 phpmyadmin:latest
sudo docker run --name dotnetcore --restart always -p 80:80 -d dotnetcore:latest
